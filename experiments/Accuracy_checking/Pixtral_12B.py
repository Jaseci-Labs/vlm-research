...
def prep_train_model(model, tokenizer, dataset, num_epochs = 5):
...
def get_response(test_dataset, model, tokenizer, n):
    ...
    output_id = model.generate(**inputs, streamer=text_streamer, max_new_tokens=64,
...
def main():
    ...
    precision = response_dict["precision"] / 10
    recall = response_dict["recall"] / 10
    f1 = response_dict["f1"] / 10
